...
int main(int argc, char** argv)
{
	...
	sdds::Collection<sdds::Book> library("Bestsellers");
	{
		...
		std::string line;
		size_t numBooksStored = 0;
		std::ifstream booksFile(argv[1]);

		if (booksFile.good()) {
			while (numBooksStored < 4) {
				std::getline(booksFile, line);
				if (!line.empty()) {
					if (line.at(0) != '#') {
						Book tmp(line);
						library += tmp;
	...
	Movie movies[5];
	{
		...
		//       - lines that start with "#" are considered comments and should be ignored
		size_t numMovieStored = 0;

		std::ifstream moviesFile(argv[2]);
		std::string line;

		if (moviesFile.good()) {
			while (numMovieStored < 5) {
				std::getline(moviesFile, line);

				if (line.at(0) != '#') {
					movies[numMovieStored] = sdds::Movie(line);
					numMovieStored++;
				}
			}
		}
		...
		for (auto i = 3u; i <= 4u; ++i)
		{
			...
			try {
				SpellChecker spelling(argv[i]);
				for (auto j = 0u; j < theCollection.size(); ++j)
					theCollection[j].fixSpelling(spelling);
				for (auto j = 0u; j < library.size(); ++j)
					library[j].fixSpelling(spelling);
			}
